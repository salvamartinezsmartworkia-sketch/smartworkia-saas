...
export function OperationalModule({ view, overview, query, setQuery, quickFilter, onClearQuickFilter, onCreate, onEdit, onOpenRecord, onOpenShipment, onRetire, onNavigate, onOpenDocument, onDeleteDocument }) {
  ...
  if (view === "orders") {
    ...
    overview.orderLines.rows.forEach((line) => {
      const current = orderLinesByOrder.get(line.purchase_order_id) || [];
      current.push(line);
      orderLinesByOrder.set(line.purchase_order_id, current);
    });
    ...
    return <section className={`${styles.panel} ${styles.ordersModule}`}><QuickFilterBanner quickFilter={quickFilter} count={rows.length} onClear={onClearQuickFilter} /><div className={styles.moduleStats}>{orderStats.map(([value, label, filter]) => <button type="button" key={label} className={quickFilter?.key === filter.key ? styles.statActive : ""} onClick={() => onNavigate("orders", false, filter)}><span>{label}</span><strong>{value}</strong></button>)}</div><ListTools query={query} setQuery={setQuery} placeholder="Pedido, proveedor, origen o material…">{statusFilter}<button type="button" className={styles.secondary} onClick={() => onNavigate("import")}>Importar Excel</button></ListTools>{rows.length ? <TableShell><thead><tr><th>Pedido</th><th>Proveedor</th><th>Origen / Incoterm</th><th>Disponibilidad</th><th>Estado</th><th>Contenido</th><th>Acciones</th></tr></thead><tbody>{rows.map((row) => { const lines = orderLinesByOrder.get(row.id) || []; const units = lines.reduce((sum, line) => sum + Number(line.quantity || 0), 0); return <tr key={row.id}><td><button className={styles.recordLink} onClick={() => onOpenRecord("orders", row)}><b>{row.order_number}</b><small>{fmtDate(row.order_date)}</small></button></td><td>{suppliers.get(row.supplier_id)?.name || "Sin proveedor"}</td><td>{row.origin || suppliers.get(row.supplier_id)?.city || "—"}<small>{suppliers.get(row.supplier_id)?.incoterm || "Incoterm pendiente"}</small></td><td>{fmtDate(row.ready_date)}</td><td><StatusBadge value={row.status} /></td><td>{lines.length} líneas<small>{units.toLocaleString("es-ES")} unidades</small></td><td><ActionButtons onOpen={() => onOpenRecord("orders", row)} onEdit={() => onEdit("orders", row)} onRetire={() => onRetire("orders", row)} retireLabel="Archivar" /></td></tr>; })}</tbody></TableShell> : <EmptyState icon={Package} title="No hay pedidos en este filtro" text="Limpia el filtro para ver todos los pedidos registrados." action={quickFilter ? "Ver todos" : "Nuevo pedido"} onAction={quickFilter ? onClearQuickFilter : () => onCreate("orders")} secondary={!quickFilter ? "Importar Excel" : null} onSecondary={() => onNavigate("import")} />}</section>;
  }
