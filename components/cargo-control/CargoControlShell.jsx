...
function DataTable({ kind, rows, onOpen, onEdit, onDelete, extraAction, showType = false }) {
  ...
  return <div className={styles.tableWrap}><table><thead><tr><th>Referencia</th><th>Información</th>{showType && <th>Tipo</th>}<th>Estado</th><th>Actualización</th><th>Acciones</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row.__kind || kind}-${row.id}`}><td><button className={styles.recordLink} onClick={() => onOpen(row)}><b>{titleOf(row)}</b><small>{LABELS[row.mode] || row.country || row.carrier || "Cargo Control"}</small></button></td><td>{row.origin || row.origin_port || row.contact_name || "—"}{(row.destination || row.destination_port) && <small> → {row.destination || row.destination_port}</small>}</td>{showType && <td><span className={`${styles.typeBadge} ${row.__kind === "forwarders" ? styles.forwarderBadge : styles.supplierBadge}`}>{row.__kind === "forwarders" ? "Transitario" : "Proveedor"}</span></td>}<td><span className={styles.badge}>{row.status || (row.active === false ? "inactivo" : "activo")}</span></td><td>{fmtDate(row.updated_at || row.created_at)}</td><td><div className={styles.rowActions}>{extraAction && <button className={styles.prominentAction} onClick={() => extraAction(row)}><Send />Solicitar</button>}{kind !== "documents" && <button onClick={() => onEdit(row)}><Pencil />Editar</button>}<button className={styles.danger} onClick={() => onDelete(row)}><Trash2 />Eliminar</button></div></td></tr>)}</tbody></table></div>;
}
...
export default function CargoControlShell() {
  ...
  const rows = useMemo(() => {
    ...
    ].filter((row) => partnerFilter === "all" || row.__kind === partnerFilter)
  ...
  }, [overview, view, query, activeShipments, showPastVessels, partnerFilter]);
  ...
  return <div className={styles.app}>
    ...
    <main className={styles.main}><header className={styles.topbar}><div><span>Operaciones</span><ArrowRight /><b>{NAV.find(([key]) => key === view)?.[1]}</b></div><div><span>{workspace.organization.name} · {workspace.membership.role}</span><button onClick={refresh} disabled={state.loading}><RefreshCw className={state.loading ? styles.spin : ""} />Actualizar</button></div></header>
      <div className={styles.workspace}>
        ...
        {["orders", "requests", "vessels", "shipments", "forecasts", "partners", "quotes", "documents"].includes(view) && <section className={styles.panel}><div className={styles.listHeader}><div><h2>{NAV.find(([key]) => key === view)?.[1]}</h2><p>{rows.length} registros visibles</p></div>{view === "quotes" && <a className={styles.templateDownload} href="/plantilla-cotizaciones.xlsx" download><Download />Descargar plantilla</a>}{view === "vessels" && <a className={styles.templateDownload} href="/plantilla-proximos-barcos.xlsx" download><Download />Descargar plantilla</a>}{view === "vessels" && <div className={styles.segmented}><button className={!showPastVessels ? styles.segmentActive : ""} onClick={() => setShowPastVessels(false)}>Próximos barcos</button><button className={showPastVessels ? styles.segmentActive : ""} onClick={() => setShowPastVessels(true)}>Barcos pasados</button></div>}{view === "partners" && <div className={styles.segmented}><button className={partnerFilter === "all" ? styles.segmentActive : ""} onClick={() => setPartnerFilter("all")}>Todos <b>{overview.suppliers.count + overview.forwarders.count}</b></button><button className={partnerFilter === "suppliers" ? styles.segmentActive : ""} onClick={() => setPartnerFilter("suppliers")}>Proveedores <b>{overview.suppliers.count}</b></button><button className={partnerFilter === "forwarders" ? styles.segmentActive : ""} onClick={() => setPartnerFilter("forwarders")}>Transitarios <b>{overview.forwarders.count}</b></button></div>}<label className={styles.search}><Search /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar referencia, puerto, proveedor…" /></label></div><DataTable kind={openKind} rows={rows} showType={view === "partners"} onOpen={(row) => view === "shipments" || view === "forecasts" ? setSelectedShipment(row) : view === "documents" ? getCargoDocumentUrl(row.storage_path).then((url) => window.open(url, "_blank", "noopener,noreferrer")) : setEditor({ kind: row.__kind || openKind, row })} onEdit={(row) => setEditor({ kind: row.__kind || openKind, row })} onDelete={(row) => removeRecord(row.__kind || openKind, row)} extraAction={view === "vessels" && !showPastVessels ? (vessel) => setEditor({ kind: "requests", row: { ...blankFor("requests", workspace.user.id), vessel_id: vessel.id, forwarder_id: vessel.forwarder_id || "", origin: vessel.origin_port, destination: vessel.destination_port, requested_pickup_date: vessel.etd || "", requested_delivery_date: vessel.eta || "" } }) : null} /></section>}
