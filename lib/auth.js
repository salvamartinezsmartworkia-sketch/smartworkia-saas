...
export const PLAN_COOKIE_NAME = "swia_plan";
...
export function enablePlanAccessCookie(plan) {
  const normalizedPlan = String(plan || "starter").trim().toLowerCase();
  setCookie(PLAN_COOKIE_NAME, normalizedPlan || "starter");
}
...
export function clearPlanAccessCookie() {
  clearCookie(PLAN_COOKIE_NAME);
}
