...
export async function saveShipmentOrders({ organizationId, shipmentId, orderIds }) {
  const { data: lines, error: linesError } = await supabase
    .from("cargo_purchase_order_lines")
    .select("id, purchase_order_id, quantity")
    .eq("organization_id", organizationId)
    .in("purchase_order_id", orderIds.length ? orderIds : ["00000000-0000-0000-0000-000000000000"]);
  if (linesError) throw linesError;

  const { data: previous, error: previousError } = await supabase
    .from("cargo_shipment_lines")
  ...
  if (nextRows.length) {
    const { error: insertError } = await supabase.from("cargo_shipment_lines").insert(nextRows);
    if (insertError) {
      const restoreRows = (previous || []).map((line) => ({
        organization_id: organizationId,
        shipment_id: shipmentId,
        purchase_order_line_id: line.purchase_order_line_id,
        quantity: line.quantity,
      }));
      if (restoreRows.length) await supabase.from("cargo_shipment_lines").insert(restoreRows);
      throw insertError;
    }
  }
  ...
  if (orderIds.length) {
    const { error: orderError } = await supabase
      .from("cargo_purchase_orders")
      .update({ status: "booked" })
      .eq("organization_id", organizationId)
      .in("id", orderIds)
      .not("status", "in", "(in_transit,delivered,cancelled,archived)");
...
export async function uploadCargoDocument({ organizationId, userId, file, entityType, entityId, documentType = null }) {
  ...
  const { data, error } = await supabase.from("cargo_documents").insert({
    ...
    entity_id: entityId, uploaded_by: userId, document_type: documentType,
  }).select().single();
  if (error) {
