...
export async function getRequestUserContext(request) {
  ...
  let profile = await readProfileForUser(scopedClient, user.id);
  const adminClient = createPrivilegedSupabaseClient();

  if (adminClient) {
    try {
      const {
        data: { user: privilegedUser },
        error: privilegedUserError,
      } = await adminClient.auth.admin.getUserById(user.id);

      if (!privilegedUserError && privilegedUser) {
        resolvedUser = privilegedUser;
      }
    } catch {
      resolvedUser = user;
    }
    ...
    if (!profile) {
      profile = await readProfileForUser(adminClient, user.id);
    }
  }
  ...
  return {
    ...
    user: resolvedUser,
    profile,
    access: buildUserAccess(resolvedUser, profile),
  };
}
